...
const CustomSelect: React.FC<{ options: {value: string, label: string}[], value: string, onChange: (value: string) => void }> = ({ options, value, onChange }) => {
    ...
    return ( <div className="relative" ref={dropdownRef}> <button type="button" onClick={() => setIsOpen(!isOpen)} className="w-full bg-white text-left p-2.5 border border-slate-300 focus:outline-none focus:ring-1 focus:ring-[var(--accent-color)] flex justify-between items-center h-[42px] rounded-full px-5"> <span className="text-slate-800">{selectedLabel}</span> <svg className={`w-5 h-5 text-slate-400 transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7" /></svg> </button> {isOpen && ( <ul className="absolute z-10 mt-1 w-full bg-white max-h-60 overflow-auto border border-[#bfdbfe]/50 rounded-2xl overflow-hidden">{options.map(option => <li key={option.value} onClick={() => { onChange(option.value); setIsOpen(false); }} className={`px-4 py-2 text-sm text-slate-700 hover:bg-emerald-50 cursor-pointer ${value === option.value ? 'bg-emerald-50' : ''}`}>{option.label}</li>)}</ul>)} </div> );
};
...
const AffiliateMultiSelect: React.FC<{ options: Affiliate[], selectedAccountIds: string[], onChange: (selected: string[]) => void }> = ({ options, selectedAccountIds, onChange }) => {
    ...
    useEffect(() => { const handleClickOutside = (event: MouseEvent) => { if (dropdownRef.current && !dropdownRef.current.contains(event.target as Node)) setIsOpen(false); }; document.addEventListener('mousedown', handleClickOutside); return () => document.removeEventListener('mousedown', handleClickOutside); }, []);
    return ( <div className="relative" ref={dropdownRef}> <button type="button" onClick={() => setIsOpen(!isOpen)} className="w-full bg-white text-left p-2.5 border border-slate-300 focus:outline-none focus:ring-1 focus:ring-[var(--accent-color)] flex justify-between items-center h-[42px] rounded-full px-5"> <span className={selectedAccountIds.length > 0 ? 'text-slate-800' : 'text-slate-400'}>{selectedAccountIds.length === 0 ? 'All Affiliates' : selectedAccountIds.length === 1 ? options.find(o => o.accountId === selectedAccountIds[0])?.name : `${selectedAccountIds.length} affiliates selected`}</span> <svg className={`w-5 h-5 text-slate-400 transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7" /></svg> </button> {isOpen && ( <div className="absolute z-10 mt-1 w-full bg-white rounded-2xl border border-[#bfdbfe]/50 overflow-hidden"><div className="p-2 border-b border-slate-200"><input type="text" placeholder="Search..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full p-2 border border-slate-300 focus:ring-1 focus:ring-[var(--accent-color)] rounded-full px-4"/></div><ul className="max-h-60 overflow-auto">{filteredOptions.map(option => ( <li key={option.accountId} onClick={() => toggleOption(option.accountId)} className="px-4 py-2 text-sm text-slate-700 hover:bg-emerald-50 cursor-pointer flex items-center"><input type="checkbox" readOnly checked={selectedAccountIds.includes(option.accountId)} className="h-4 w-4 text-[var(--accent-color)] border-slate-300 mr-3 focus:ring-[var(--accent-color)]" />{option.name}</li>))}</ul></div>)} </div> );
};
    ...
    return ( 
        <div className="card p-6 space-y-6"> 
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6"> 
                <div className="col-span-1"><label className="block text-sm font-medium text-slate-700 mb-1">Time Range</label><DateRangePicker value={dateRange} onChange={setDateRange} onPresetSelect={handlePresetSelect} /></div> 
...
const PerformanceOverview: React.FC = () => {
    ...
    const handleGetMetrics = useCallback(async () => {
        setLoading(true); setError(null); setDailyData(null); setShowDailyTable(false); setMerchantMetrics(null);
        try {
    ...
    const MetricBlock = ({ title, value, unit = '', changeText, description, colorClass = 'text-slate-800', hoverTooltip }: any) => (
        <div className="bg-white p-6 rounded-xl border border-[#bfdbfe]/50 flex flex-col justify-between transition-all duration-300">
            <div>
                ...
                <h4 title={hoverTooltip} className={`text-4xl font-extrabold ${colorClass} tracking-tight my-2 ${hoverTooltip ? 'cursor-help border-b border-dashed border-slate-300 inline-block pb-0.5' : ''}`}>
            ...
            {description && (
                <div className="mt-4 pt-4 border-t border-slate-50 text-xs text-slate-400 flex items-start">
