...
const SearchableSelect: React.FC<SearchableSelectProps> = ({ options, value, onChange, placeholder = "Select an option" }) => {
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            <button
                type="button"
                className="w-full bg-white text-left p-2 universal-border focus:outline-none focus:ring-2 focus:ring-[#23C48C] flex justify-between items-center h-[42px]"
                onClick={() => setIsOpen(!isOpen)}
            >
                <span className={selectedOption ? 'text-slate-800' : 'text-slate-400'}>
                    {selectedOption ? `${selectedOption.name} (${selectedOption.publicId})` : placeholder}
                </span>
                 <svg className="w-5 h-5 text-slate-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 9l4-4 4 4m0 6l-4 4-4-4" /></svg>
            </button>

            {isOpen && (
                <div className="absolute z-10 mt-[-1px] w-full bg-white max-h-60 overflow-hidden universal-border">
                    <div className="p-2 border-b border-[var(--border-color)]">
                        <input
                            type="text"
                            placeholder="Search affiliates..."
                            className="w-full px-3 py-2 universal-border focus:outline-none focus:ring-1 focus:ring-[#23C48C]"
                            value={searchTerm}
                    ...
                    <ul className="overflow-auto max-h-48">
                        {filteredOptions.map(option => (
                            <li
                                key={option.publicId}
                                className={`px-4 py-2 text-sm text-slate-700 hover:bg-green-50 cursor-pointer ${value === option.publicId ? 'bg-green-100' : ''}`}
...
const PartnerAnalysis: React.FC = () => {
    ...
    return (
        <div className="space-y-6">
            <div className="bg-white p-4 universal-border universal-shadow flex flex-col md:flex-row items-end gap-4">
                <div className="w-full md:w-auto">
                    ...
                    <input
                        ...
                        className="mt-1 p-2 universal-border focus:ring-[#23C48C] focus:border-[#23C48C] h-[42px]"
                    />
                </div>
                <div className="w-full md:w-1/3">
                    <label htmlFor="affiliate" className="block text-sm font-medium text-slate-700">Affiliate</label>
                     <SearchableSelect
                ...
                <div className="w-full md:w-auto">
                    <button onClick={handleGetMetric} disabled={loading || !selectedAffiliate} className="w-full px-6 py-2.5 text-white font-semibold primary-btn bg-[#23C48C] universal-border focus:outline-none disabled:bg-slate-400 disabled:cursor-not-allowed disabled:transform-none">
            ...
            {loading && <Loader />}
            {error && <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 universal-border" role="alert">{error}</div>}
            {data && (
                <>
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div className="bg-white p-4 universal-border universal-shadow"><p className="text-sm text-slate-500">Total Clicks</p><p className="text-2xl font-bold text-slate-800">{formatNumber(data.clicks)}</p></div>
                        <div className="bg-white p-4 universal-border universal-shadow"><p className="text-sm text-slate-500">Total Installs</p><p className="text-2xl font-bold text-slate-800">{formatNumber(data.installs)}</p></div>
                        <div className="bg-white p-4 universal-border universal-shadow"><p className="text-sm text-slate-500">Total Revenue</p><p className="text-2xl font-bold text-slate-800">{formatCurrency(data.revenue)}</p></div>
                        <div className="bg-white p-4 universal-border universal-shadow"><p className="text-sm text-slate-500">Total Payout</p><p className="text-2xl font-bold text-slate-800">{formatCurrency(data.payout)}</p></div>
                    </div>
                    
                    <div className="bg-white p-4 universal-border universal-shadow overflow-x-auto">
                        <h3 className="text-lg font-semibold text-slate-800 mb-4">Daily Breakdown</h3>
                        {data.daily.length > 0 ? (
                            <table className="w-full text-sm text-left"><thead className="text-xs text-slate-700 uppercase bg-slate-100">
                                    <tr>
                                ...
                                <tbody>
                                    {data.daily.map(row => (
                                        <tr key={row.date} className="bg-white border-b border-[var(--border-color)] hover:bg-slate-50">
                                            <td className="px-6 py-4 font-medium text-slate-900">{row.date}</td>
                                            <td className="px-6 py-4 text-right">{formatNumber(row.clicks)}</td>
                                            <td className="px-6 py-4 text-right">{formatNumber(row.installs)}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
