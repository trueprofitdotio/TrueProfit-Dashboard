    ...
    const handleGetMetrics = useCallback(async () => {
        ...
        try {
            ...
            const finalData: MerchantDetailsData[] = customerIdsInPeriod.map(customerId => { 
                ...
                const hasPayout = merchant.payoutCount !== undefined && merchant.payoutCount > 0;
                return { customerId, affiliateId, affiliateName: affiliateNameMap.get(affiliateId) || 'Unknown', revenueInPeriod: merchant.revenueInPeriod || 0, payoutInPeriod: merchant.payoutInPeriod || 0, status: hasPayout ? 'Paying' : 'Install', installedDate: merchant.installedDate || null, lastPayoutDate: merchant.lastPayoutDate || null, merchantLifetime: merchantLifetime, merchantLifetimeDays: merchantLifetimeDays, totalLifetimeRevenue: totalRevenue, totalLifetimePayout: merchant.totalLifetimePayout || 0, allConversions: merchant.allConversions || [], payoutCount: merchant.payoutCount || 0 }; 
            });
            ...
            // Note: 'paying' for previous period is an approximation based on payouts in that period, as lifetime data isn't fetched for those customers.
            const prevCustomerIds = new Set(prevConversions.rows.map(c => c.customerId).filter(Boolean));
            const prevConversionsByCustomer = new Map<string, number>();
            prevConversions.rows.forEach(c => {
                if (c.customerId && c.conversionType.name.toLowerCase() === 'payout') {
                    const currentCount = prevConversionsByCustomer.get(c.customerId) || 0;
                    prevConversionsByCustomer.set(c.customerId, currentCount + 1);
                }
            });
            const prevPayingCustomerCount = Array.from(prevConversionsByCustomer.values()).filter(count => count > 0).length;
