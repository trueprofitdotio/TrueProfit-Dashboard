...
const MerchantsDetails: React.FC = () => {
    ...
    const handleGetMetrics = useCallback(async () => {
        ...
        try {
            const affiliateNameMap = new Map<string, string>(); allAffiliates.forEach(aff => affiliateNameMap.set(aff.publicId, aff.name));
            const filteredPublicIds = selectedAffiliates.length > 0 ? allAffiliates.filter(a => selectedAffiliates.includes(a.accountId)).map(a => a.publicId) : undefined;
            ...
            for (const [customerId, conversions] of conversionsByCustomer.entries()) {
                if (conversions.length === 0) continue; conversions.sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime()); const merchant: TempMerchantData = { customerId, allConversions: conversions, totalLifetimeRevenue: 0, totalLifetimePayout: 0, installedDate: null, lastPayoutDate: null, affiliateId: conversions[0].source?.publicId || 'N/A' };
                for (const conv of conversions) { merchant.totalLifetimeRevenue += parseFloat(conv.revenue.value || '0'); merchant.totalLifetimePayout += parseFloat(conv.cost.value || '0'); if (conv.conversionType.name.toLowerCase() === 'install' && !merchant.installedDate) merchant.installedDate = conv.createdAt; if (conv.conversionType.name.toLowerCase() === 'payout') merchant.lastPayoutDate = conv.createdAt; }
            ...
            const finalData: MerchantDetailsData[] = customerIdsInPeriod.map(customerId => { const merchant = merchantsMap.get(customerId)!; let merchantLifetime: string | null = null; const affiliateId = merchant.affiliateId!; if (merchant.installedDate && merchant.lastPayoutDate) { const lifetimeInMs = new Date(merchant.lastPayoutDate).getTime() - new Date(merchant.installedDate).getTime(); if (lifetimeInMs >= 0) merchantLifetime = `${Math.round(lifetimeInMs / 864e5)} days`; } const totalRevenue = merchant.totalLifetimeRevenue || 0; return { customerId, affiliateId, affiliateName: affiliateNameMap.get(affiliateId) || 'Unknown', revenueInPeriod: merchant.revenueInPeriod || 0, payoutInPeriod: merchant.payoutInPeriod || 0, status: totalRevenue > 0 ? 'Paying' : 'Install', installedDate: merchant.installedDate || null, lastPayoutDate: merchant.lastPayoutDate || null, merchantLifetime: merchantLifetime, totalLifetimeRevenue: totalRevenue, totalLifetimePayout: merchant.totalLifetimePayout || 0, allConversions: merchant.allConversions || [], }; });
    ...
    const sortedMerchantsData = useMemo(() => { const sortableItems = [...merchantsData]; if (sortConfig.key !== null) { sortableItems.sort((a, b) => { const aValue = a[sortConfig.key!]; const bValue = b[sortConfig.key!]; if (aValue === null || aValue === undefined) return 1; if (bValue === null || bValue === undefined) return -1; if (typeof aValue === 'number' && typeof bValue === 'number') { return sortConfig.direction === 'ascending' ? (aValue as number) - (bValue as number) : (bValue as number) - (aValue as number); } else { const strA = String(aValue).toLowerCase(); const strB = String(bValue).toLowerCase(); if (strA < strB) return sortConfig.direction === 'ascending' ? -1 : 1; if (strA > strB) return sortConfig.direction === 'ascending' ? 1 : -1; return 0; } }); } return sortableItems; }, [merchantsData, sortConfig]);
